 ...
 * $Date: 18/05/31 4:57p $
 * @brief
 *           Demonstrate how to set I2C double buffer in slave mode to receive the
 *           data from a Master.
...
#include <stdio.h>
#include "NuMicro.h"
#define  I2C_ENABLE_TWOBUFEN(i2c)   ((i2c)->CTL1 |= I2C_CTL1_TWOBUFEN_Msk)
#define  I2C0_CLK 100000
#define  I2C0_Address0 0x15
#define  I2C0_Address1 0x35
#define  I2C0_Address2 0x55
#define  I2C0_Address3 0x75
/*---------------------------------------------------------------------------------------------------------*/
/* Global variables                                                                                        */
/*---------------------------------------------------------------------------------------------------------*/
...
void I2C_SlaveTRx(uint32_t u32Status)
{
    ...
    else if(u32Status == 0x80)                 /* Previously address with own SLA address                                                   Data has been received; ACK has been returned*/
    {
        ...
        //check i2c buffer is empty and reciver buffer data
        if(I2C0->CTL1&I2C_STATUS1_EMPTY_Msk!=0)
        {
            u8data = (unsigned char) I2C_GET_DATA(I2C0);
            if(g_u8SlvDataLen < 2)
            {
    ...
    else if(u32Status == 0xA8)                  /* Own SLA+R has been receive; ACK has been return */
    {
        ...
        slave_buff_addr++;
        I2C_SET_CONTROL_REG(I2C0, I2C_CTL_SI_AA);
        //check i2c buffer is full and send data to buffer
        if(I2C0->CTL1&I2C_STATUS1_FULL_Msk!=1)
        {
            I2C_SET_DATA(I2C0, g_au8SlvData[slave_buff_addr]);
            slave_buff_addr++;
        }
...
void I2C0_Init(void)
{
    ...
    I2C_Open(I2C0, I2C0_CLK);

    /* Get I2C0 Bus Clock */
    printf("I2C clock %d Hz\n", I2C_GetBusClockFreq(I2C0));

    /* Set I2C Slave Addresses */
    I2C_SetSlaveAddr(I2C0, 0, I2C0_Address0, 0);   /* Slave Address : 0x15 */
    I2C_SetSlaveAddr(I2C0, 1, I2C0_Address1, 0);   /* Slave Address : 0x35 */
    I2C_SetSlaveAddr(I2C0, 2, I2C0_Address2, 0);   /* Slave Address : 0x55 */
    I2C_SetSlaveAddr(I2C0, 3, I2C0_Address3, 0);   /* Slave Address : 0x75 */
...
int main()
{
    ...
    */
    ...
    /*Enable I2C_Double_Buffer_Slave*/
    I2C_ENABLE_TWOBUFEN(I2C0);

    /* I2C enter no address SLV mode */
    I2C_SET_CONTROL_REG(I2C0, I2C_CTL_SI_AA);

    //clear g_au8SlvData array
    for(i = 0; i < 0x100; i++)
    {
        g_au8SlvData[i] = 0;
    }
