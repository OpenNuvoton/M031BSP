...
* $Revision: 12 $
* $Date: 18/07/18 4:52p $
* @brief    M031 series USB composite device sample file
*
* @note
    ...
    if (buf[0] & EP_INPUT)  /* request data transfer direction */
    {
        ...
        switch (buf[1])
        {
            case GET_MAX_LUN:
            {
                ...
                if ((buf[4] == gsInfo.gu8ConfigDesc[LEN_CONFIG + LEN_INTERFACE + LEN_HID + 2 * LEN_ENDPOINT + 2]) && (buf[4] == 0) && (buf[6] == 1) )
                {
...
//             }
    ...
    else
    {
        ...
        switch (buf[1])
        {
            ...
            {
                /* Check interface number with cfg descriptor and check wValue = 0, wIndex = 1, wLength = 0
                */ 
                if(buf[4] == 0x01)
                {
                    USBD_SET_DATA1(EP0);
