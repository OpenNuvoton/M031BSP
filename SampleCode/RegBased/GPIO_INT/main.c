...
int main(void)
{
    ...
    printf("    PC.5 is falling edge trigger.\n");
    ...
    PB->INTEN |= (BIT2<<16);    // enable  rising  edge trigger interrupt
    PB->INTEN &= ~BIT2;         // disable falling edge trigger interrupt
    ...
    /* Configure interrupt mode of specified pin */
    PC->INTTYPE &= ~BIT5;   // set PC5 to edge trigger interrupt
    /* Enable interrupt function of specified pin */
    PC->INTEN &= ~(BIT5<<16);   // disable rising  edge trigger interrupt
    PC->INTEN |= BIT5;          // enable  falling edge trigger interrupt
