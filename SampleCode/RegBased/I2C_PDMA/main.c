...
void PDMA_Init(void)
{
    ...
    PDMA->CHCTL |= (1 << I2C_PDMA_TX_CH); // Channel 0 for I2C0 TX
    PDMA->CHCTL |= (1 << I2C_PDMA_RX_CH); // Channel 1 for I2C1 RX

    /* PDMA Settings */
    PDMA->DSCT[I2C_PDMA_TX_CH].CTL =
        PDMA_OP_BASIC |            /* Basic Mode */
        PDMA_REQ_SINGLE |          /* single transfer type */
        PDMA_SAR_INC |             /* Source address -> incremented(g_u8Tx_Buffer[n]) */
        PDMA_DAR_FIX |             /* Destination address -> fixed(I2C0_DAT) */
        PDMA_WIDTH_8 |             /* transfer width -> 8-bit */
        ((PDMA_TEST_LENGTH - 1) << 16); /* transfer count */
    PDMA->DSCT[I2C_PDMA_TX_CH].SA = (uint32_t)(&g_u8Tx_Buffer[0]);
    PDMA->DSCT[I2C_PDMA_TX_CH].DA = (uint32_t)(&(I2C0->DAT));

    PDMA->DSCT[I2C_PDMA_RX_CH].CTL =
        PDMA_OP_BASIC |            /* Basic mode */
        PDMA_REQ_SINGLE |          /* Single transfer type */
        PDMA_SAR_FIX  |            /* source address -> Fix(I2C1_DAT) */
        PDMA_DAR_INC  |            /* destination address -> incremented(g_u8Rx_Buffer[n]) */
        PDMA_WIDTH_8 |             /* transfer width -> 8-bit */
        ((PDMA_TEST_LENGTH - 1) << 16); /* transfer count */
    PDMA->DSCT[I2C_PDMA_RX_CH].SA = (uint32_t)(&(I2C1->DAT));
    PDMA->DSCT[I2C_PDMA_RX_CH].DA = (uint32_t)(&g_u8Rx_Buffer[0]);
    ...
    PDMA->INTEN |= (1 << I2C_PDMA_TX_CH);
    /* Enable PDMA Channel 1 INT */
    PDMA->INTEN |= (1 << I2C_PDMA_RX_CH);
