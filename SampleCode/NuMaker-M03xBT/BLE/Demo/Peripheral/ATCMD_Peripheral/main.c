...
#else
uint8_t uartBuffer[48];
#endif
char atcmdBuffer[48];

uint8_t atcmd_enabled;                                //ATCMD or DATA mode
uint8_t echo_enabled = 1;                             //ATCMD echo mode
uint8_t sleep_enabled = 0;                            //MCU enter to sleep or not
volatile uint8_t dsleep_enabled = 0;                  //MCU & RF enter to deep-sleep
...
/* The PA/PB/PG/PH default IRQ, declared in startup_M031Series.s */
void GPABGH_IRQHandler(void)
{
    volatile uint32_t temp;

    /* To check if PB.1 interrupt occurred */
    if (GPIO_GET_INT_FLAG(PB, BIT1))
    {
        GPIO_CLR_INT_FLAG(PB, BIT1);
        atcmd_enabled = PB1;
        debug_printf("atcmd_enabled = %d\n", atcmd_enabled);
    }
    else
    {
...
void GPIO_Init(void)
{
    /* Configure PB.1 as Input mode and enable interrupt by both edge trigger */
...
int main(void)
{
    ...
    /* Set default debug baudrate to 115200 */
    ...
    if (GET_CHIP_SERIES_NUM == CHIP_SERIES_NUM_G)
    {
        ...
        default_level = TXPOWER_8DBM;
    }
    else if (GET_CHIP_SERIES_NUM == CHIP_SERIES_NUM_I)
    {
        /* For 256 KB chipset, the maximum power level is 7 dBm */
        default_level = TXPOWER_7DBM;
    }

    /* Reset the default TX Power level */
    for (i = 0; i < 3; i++)
    {
        ble_tx_power_def[i] = default_level;
    }
#endif

    /* Load settings from data flash */
    atcmd_set_data_flash();
    ...
    printf("------------------------------------------\n");
    printf("  BLE demo: ATCMD(Peripheral) start...\n");
    printf("------------------------------------------\n");
    ...
    while (1)
    {
        ...
        if (setBLE_KernelStateHandle() == BLESTACK_STATUS_FREE)
        {
            ...
            if ((sleep_enabled) || (dsleep_enabled))
            {
...
void UART02_IRQHandler(void)
{
    ...
    if (u32IntSts & UART_INTSTS_RDAINT_Msk)
    {
        if (dsleep_enabled)
        {
            status = setRF_WakeUpFromDeepSleep();
            debug_printf("setRF_WakeUpFromDeepSleep() = %d\n", status);
            if (status == BLESTACK_STATUS_SUCCESS)
            {
                /* Restore BLE settings after wakeup */
                Ble_Slave_Init();
                dsleep_enabled = 0;
            }
        }
        ...
        while (UART_IS_RX_READY(UART0))
        {
            ...
            /* Only do data transfer in BLE connection */
            if ((atcmd_enabled == 0) && (bleProfile_link0_info.bleState == STATE_BLE_CONNECTION))
            {
                if (index >= TRSPX_mtu)
                {
            ...
            else
            {
                ...
                if ((uartBuffer[index - 1] == '\r') || (uartBuffer[index - 1] == '\n'))
                {
                    ...
                    if (atcmd_state != AT_STATE_FREE)
                    {
                        /* AT command is not ready */
                        if (index > 1)
                            printf("BUSY\r\n");
                        index = 0;
                    }
                    ...
                    {
                        memset(atcmdBuffer, 0x0, sizeof(atcmdBuffer));

                        if (index < (sizeof(atcmdBuffer) - 1))
                            strncpy(atcmdBuffer, (char *)uartBuffer, index);
                        else
                            strncpy(atcmdBuffer, (char *)uartBuffer, sizeof(atcmdBuffer) - 1);

                        if (index > 1)
                            atcmd_handler(atcmdBuffer);
                        index = 0;
                    }
                }
                else if ((uartBuffer[index - 1] == 0x08) || (uartBuffer[index - 1] == 0x7f))
                {
                    /* Do back space */
                    if (echo_enabled)
                    {
                        printf("%c%c", 0x20, 0x08);
                    }
                    index--;
                    uartBuffer[index] = 0;

                    /* If there is still a character to delete */
                    if (index > 0)
                    {
                        index--;
                        uartBuffer[index] = 0;
